...
class TouchLocationTest
{
    [Test]
    [Description(
        "AgeState is used to make touches that were placed and removed on the same frame appear over multiple frames when no actual update happens on the second frame. When it does this it should copy its details so they can be fetched from the previous state")]
