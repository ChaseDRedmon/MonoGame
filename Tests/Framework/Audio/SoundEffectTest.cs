...
public class SoundEffectTests
{
    ...
    public void GetSampleDuration()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleDuration(2, 8000 - 1, AudioChannels.Mono));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleDuration(2, 8000, AudioChannels.Mono));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleDuration(2, 48000, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleDuration(2, 48000 + 1, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleDuration(2, int.MaxValue, AudioChannels.Mono));

        // Test channel range.
        Assert.Throws<ArgumentOutOfRangeException>(
            () => SoundEffect.GetSampleDuration(2, 8000, (AudioChannels)(-1)));
        Assert.Throws<ArgumentOutOfRangeException>(() => SoundEffect.GetSampleDuration(2, 8000, 0));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleDuration(2, 8000, AudioChannels.Mono));
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleDuration(2, 8000, (AudioChannels)int.MaxValue));
        ...
        Assert.AreEqual(TimeSpan.FromSeconds(1),
            SoundEffect.GetSampleDuration(192000, 48000, AudioChannels.Stereo));
    }
    ...
    public void GetSampleSizeInBytes()
    {
        // Test duration range.
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.MinValue, 8000, AudioChannels.Mono));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, AudioChannels.Mono));
        Assert.DoesNotThrow(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.FromMilliseconds(0x7FFFFFF), 8000, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.FromMilliseconds(0x7FFFFFF + 1), 8000, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.MaxValue, 8000, AudioChannels.Mono));

        // Test sampleRate range.
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, -1, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 0, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000 - 1, AudioChannels.Mono));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, AudioChannels.Mono));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 48000, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 48000 + 1, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, int.MaxValue, AudioChannels.Mono));

        // Test channel range.
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, (AudioChannels)(-1)));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, 0));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, AudioChannels.Mono));
        Assert.DoesNotThrow(() => SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, AudioChannels.Stereo));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, (AudioChannels)3));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            SoundEffect.GetSampleSizeInBytes(TimeSpan.Zero, 8000, (AudioChannels)int.MaxValue));
        ...
        Assert.AreEqual(96000,
            SoundEffect.GetSampleSizeInBytes(TimeSpan.FromSeconds(1), 48000, AudioChannels.Mono));
        Assert.AreEqual(32000,
            SoundEffect.GetSampleSizeInBytes(TimeSpan.FromSeconds(1), 8000, AudioChannels.Stereo));
        Assert.AreEqual(192000,
            SoundEffect.GetSampleSizeInBytes(TimeSpan.FromSeconds(1), 48000, AudioChannels.Stereo));
    }
    ...
    public void Ctor1()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(
            () => new SoundEffect(new byte[2], 48000 + 1, AudioChannels.Mono));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], int.MaxValue, AudioChannels.Mono));

        // Test channel range.
        Assert.Throws<ArgumentOutOfRangeException>(() => new SoundEffect(new byte[2], 8000, (AudioChannels)(-1)));
        Assert.Throws<ArgumentOutOfRangeException>(() => new SoundEffect(new byte[2], 8000, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[2], 8000, AudioChannels.Mono));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[4], 8000, AudioChannels.Stereo));
        Assert.Throws<ArgumentOutOfRangeException>(() => new SoundEffect(new byte[2], 8000, (AudioChannels)3));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 8000, (AudioChannels)int.MaxValue));
    ...
    public void Ctor2()
    {
        ...
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[0], 0, 4, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[1], 0, 4, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], 0, 4, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[3], 0, 4, 8000, AudioChannels.Stereo, 0, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[4], 0, 4, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[5], 0, 8, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[6], 0, 8, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[7], 0, 8, 8000, AudioChannels.Stereo, 0, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[8], 0, 8, 8000, AudioChannels.Stereo, 0, 0));

        // Test offset.
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], int.MinValue, 2, 8000, AudioChannels.Mono, 0, 0));
        ...
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], int.MaxValue, 2, 8000, AudioChannels.Mono, 0, 0));

        // Test count.
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], 0, int.MinValue, 8000, AudioChannels.Mono, 0, 0));
        ...
        // Test sampleRate range.
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, -1, AudioChannels.Mono, 0, 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, 0, AudioChannels.Mono, 0, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[2], 0, 2, 8000, AudioChannels.Mono, 0, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[2], 0, 2, 48000, AudioChannels.Mono, 0, 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, 48000 + 1, AudioChannels.Mono, 0, 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, int.MaxValue, AudioChannels.Mono, 0, 0));

        // Test channel range.
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, (AudioChannels)(-1), 0, 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, 0, 0, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[2], 0, 2, 8000, AudioChannels.Mono, 0, 0));
        Assert.DoesNotThrow(() => new SoundEffect(new byte[4], 0, 4, 8000, AudioChannels.Stereo, 0, 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, (AudioChannels)3, 0, 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, (AudioChannels)int.MaxValue, 0, 0));

        // Test loop start.
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, AudioChannels.Mono, int.MinValue, 0));
        ...
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, AudioChannels.Mono, int.MaxValue, 0));

        // Test loop end.
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, AudioChannels.Mono, 0, int.MinValue));
        ...
        Assert.Throws<ArgumentException>(() =>
            new SoundEffect(new byte[2], 0, 2, 8000, AudioChannels.Mono, 0, int.MaxValue));
    }
