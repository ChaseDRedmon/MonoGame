...
public class Texture3DNonVisualTest
{
    ...
    public void ZeroSizeShouldFailTest()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 0, 1, 1, false, SurfaceFormat.Color));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 1, 0, 1, false, SurfaceFormat.Color));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 1, 1, 0, false, SurfaceFormat.Color));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 0, 0, 1, false, SurfaceFormat.Color));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 1, 0, 0, false, SurfaceFormat.Color));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 0, 1, 0, false, SurfaceFormat.Color));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            texture = new Texture3D(gd, 0, 0, 0, false, SurfaceFormat.Color));
    }
    ...
    public void SetData9ParametersSuccessTest(int arrayLength, int startIndex, int elementCount, int x, int y,
        int z, int w, int h, int d)
    {
        ...
        int cx, cy, cz;
        for (int i = 0, j = 0;
             i < Texture3DNonVisualTest.w * Texture3DNonVisualTest.h * Texture3DNonVisualTest.d;
             i++)
        {
            ...
            if (cx >= x && cx < w + x && cy >= y && cy < h + y && cz >= z && cz < d + z)
                Assert.AreEqual(write[startIndex + j++], written[i],
                    string.Format("bad color in position x:{0};y:{1};z:{2};i:{3}", cx, cy, cz, i));
            else
                Assert.AreEqual(reference[i], written[i],
                    string.Format("bad color in position x:{0};y:{1};z:{2};i:{3}, outside requested area", cx, cy,
                        cz, i));
        }
    }
    ...
    public void SetData9ParametersExceptionTest(int arrayLength, int startIndex, int elementCount, int x, int y,
        int z, int w, int h, int d)
    {
        Color[] write = new Color[arrayLength];
        for (int i = startIndex; i < arrayLength; i++)
