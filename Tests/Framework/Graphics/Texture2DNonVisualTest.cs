...
internal class Texture2DNonVisualTest : GraphicsDeviceTestFixtureBase
{
    ...
    public void FromStreamShouldWorkTest(string filename)
    {
        ...
        try
        {
            var bitmap = ImageResult.FromMemory(File.ReadAllBytes(filename),
                ColorComponents.RedGreenBlueAlpha);
            Assert.AreEqual(bitmap.Height, _texture.Height);
            Assert.AreEqual(bitmap.Width, _texture.Width);
    ...
    {
        Assert.Throws<ArgumentNullException>(() => Texture2D.FromStream(gd, null));
#if !XNA
    ...
    public void PlatformGetDataWithOffsetTest(int rx, int ry, int rw, int rh, int startIndex, int elementsToRead)
    {
        using (StreamReader reader = new StreamReader("Assets/Textures/LogoOnly_64px.png"))
        {
            ...
            {
                Assert.AreNotEqual(255, colors[i + startIndex].R, "colors was not overwritten in position {0}",
                    startIndex + i);
            }
    ...
    public void SetData1ParameterGoodTest(int arraySize)
    {
        using (StreamReader reader = new StreamReader("Assets/Textures/LogoOnly_64px.png"))
        {
            ...
            for (int i = 0; i < written.Length; i++)
            {
                ...
                {
                    Assert.AreEqual(reference[i].R, written[i].R,
                        "Color written in position:{0}; beyond array data", i);
                    Assert.AreEqual(reference[i].G, written[i].G,
                        "Color written in position:{0}; beyond array data", i);
                    Assert.AreEqual(reference[i].B, written[i].B,
                        "Color written in position:{0}; beyond array data", i);
                    Assert.AreEqual(reference[i].A, written[i].A,
                        "Color written in position:{0}; beyond array data", i);
                }
            }
    ...
    public void SetDataFormatFailingTestTBufferTooLarge<TBuffer>(SurfaceFormat format, TBuffer value)
    ...
    [TestCase(3970, 1, 4096, 1, 1, 63, 63)]
    public void SetData5ParameterExceptionTest(int arraySize, int startIndex, int elements, int x, int y, int w,
        int h)
    {
        using (StreamReader reader = new StreamReader("Assets/Textures/LogoOnly_64px.png"))
        {
