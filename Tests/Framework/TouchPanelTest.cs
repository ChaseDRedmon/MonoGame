...
internal class TouchPanelTest
{
    ...
    [Test]
    [Description(
        "In XNA if you press then move your finger before GetState is called, the initial touch position is the latest position of the touch")]
    ...
    [TestCase(false)]
    [Description(
        "In XNA if you press and release your finger over multiple frames where GetState is not called, the touch never shows up")]
    ...
    [Description(
        "If you press and release a touch on the same frame we need to show it as pressed that frame and released in the next")]
    ...
    [TestCase(false)]
    [Description(
        "Press and release our finger on the same frame. Don't call GetState that frame, but do the next. We should not get the touch")]
    ...
    [Test]
    [Description(
        "Internally TouchPanelState uses a queue of TouchLocation updating state. If it gets longer than 100, it throws away the first ones, which is bad")]
    ...
    public void ReleaseAllTouchesTest(bool testBetween)
    {
        ...
        Assert.AreEqual(testBetween ? TouchLocationState.Released : TouchLocationState.Pressed,
            state.Single(p => p.Id != initialTouch.Id).State);
