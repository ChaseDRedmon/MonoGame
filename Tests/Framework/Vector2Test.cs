...
class Vector2Test
{
    ...
    public void Transform()
    {
        ...
        // TRANSFORM ON LIST (MATRIX)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Assert.That(desinationList1[i],
                    Is.EqualTo(new Vector2(19 + 6 * i, 22 + 8 * i)).Using(Vector2Comparer.Epsilon));
            }
        }
        ...
        // TRANSFORM ON LIST (QUATERNION)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Assert.That(new Vector2(-45 + -45 * i, 9 + 9 * i),
                    Is.EqualTo(desinationList4[i]).Using(Vector2Comparer.Epsilon));
            }
        }
        ...
        // TRANSFORM ON LIST (QUATERNION)(SIMPLE)
        {
            ...
            {
                Assert.That(new Vector2(-45 + -45 * i, 9 + 9 * i),
                    Is.EqualTo(desinationList6[i]).Using(Vector2Comparer.Epsilon));
            }
        }
    }
    ...
    public void TransformNormal()
    {
        ...
        // TRANSFORM ON LIST
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Assert.That(new Vector2(0 + 6 * i, 0 + 8 * i),
                    Is.EqualTo(destinationArray1[i]).Using(Vector2Comparer.Epsilon));
            }
        }
        // TRANSFORM ON LIST(SOURCE OFFSET)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Assert.That(new Vector2(30 + 6 * i, 40 + 8 * i),
                    Is.EqualTo(destinationArray2[i]).Using(Vector2Comparer.Epsilon));
            }
        ...
        // TRANSFORM ON LIST(SIMPLE)
        {
            ...
            {
                Assert.That(new Vector2(0 + 6 * i, 0 + 8 * i),
                    Is.EqualTo(destinationArray5[i]).Using(Vector2Comparer.Epsilon));
            }
        }
    }
    ...
    public void TypeConverter()
    {
        ...
        Assert.AreEqual(new Vector2(1024.5f, 2048.75f),
            converter.ConvertFromString(null, invariantCulture, "1024.5, 2048.75"));
        ...
        Assert.AreEqual("1024.5, 2048.75",
            converter.ConvertToString(null, invariantCulture, new Vector2(1024.5f, 2048.75f)));
