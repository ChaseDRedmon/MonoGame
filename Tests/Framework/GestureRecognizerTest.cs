...
class GestureRecognizerTest
{
    ...
    private const GestureType AllDrags = GestureType.DragComplete | GestureType.FreeDrag |
                                         GestureType.HorizontalDrag | GestureType.VerticalDrag;

    private const GestureType AllGestures =
        GestureType.DoubleTap | GestureType.DragComplete | GestureType.Flick | GestureType.FreeDrag |
        GestureType.Hold |
        GestureType.HorizontalDrag | GestureType.Pinch | GestureType.PinchComplete | GestureType.Tap |
    ...
    [Description(
        "Hold a finger down, then perform a tap and double tap with another, this should not make any tap gestures")]
    ...
    [Test]
    [Description(
        "If the user does a horizontal drag, this should be picked up as a free drag instead if horizontal is not enabled")]
    ...
    [Test]
    [Description(
        "Start a drag then disable gestures. Gestures events should just stop, no DragComplete gesture should happen")]
    ...
    [Test]
    [Description(
        "Start a drag then disable gestures. Re-Enable them without releasing the finger. Releasing it then should make a DragComplete")]
    ...
    [Test]
    [Description(
        "Start a drag then disable gestures. Release finger and replace, then re-enable gestures and release. Should not make a DragComplete")]
    ...
    public void ShortMovementDoesntMakeAFlick()
    {
        ...
        _tps.AddEvent(1, TouchLocationState.Released,
            startPos + new Vector2(TouchPanelState.TapJitterTolerance, 0));
        TouchPanelState.CurrentTimestamp = GameTimeForFrame(2);
    ...
    [Test]
    [Description(
        "If Flick and FreeDrag are enabled, both events should be generated without impacting each other. " +
