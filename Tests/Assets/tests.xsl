...
<xsl:stylesheet
                            ...
                            <table>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <xsl:if test="$tests.errors > 0">
                                            <td>
                                                <xsl:value-of select="$tests.errors"/>
                                            </td>
                                        </xsl:if>
                                        <xsl:if test="$tests.inconclusive > 0">
                                            <td>
                                                <xsl:value-of select="$tests.inconclusive"/>
                                            </td>
                                        </xsl:if>
                                        <xsl:if test="$tests.ignored > 0">
                                            <td>
                                                <xsl:value-of select="$tests.ignored"/>
                                            </td>
                                        </xsl:if>
                                        <xsl:if test="$tests.skipped > 0">
                                            <td>
    ...
    <xsl:template match="test-results">
        ...
        <xsl:choose>
            <xsl:when test="$failure.count > 0">
                <div class="error">
                    <xsl:attribute name="title">Failed tests:
            ...
            <xsl:when test="$ignored.count > 0">
                <div class="ignored">
                    <xsl:attribute name="title">Ignored tests:
                        <xsl:value-of select="$ignored.count"/>
                    </xsl:attribute>
                </div>
            </xsl:when>
            <xsl:otherwise>
        ...
        <div class="minus">
            <xsl:attribute name="id">img<xsl:value-of select="$test.suite.id"/>
            </xsl:attribute>
            <xsl:attribute name="onclick">javascript:toggleDiv('img<xsl:value-of select="$test.suite.id"/>', 'divDetails<xsl:value-of
                    select="$test.suite.id"/>');
            </xsl:attribute>
        </div>
        ...
        <!-- Suite details -->
        <div>
            <xsl:attribute name="id">divDetails<xsl:value-of select="$test.suite.id"/>
            </xsl:attribute>
            <blockquote>
    ...
    <xsl:template match="test-suite">
        ...
        <div>
            ...
            <div class="plus">
                <xsl:attribute name="id">imgTestCase_<xsl:value-of select="@name"/>
                </xsl:attribute>
            ...
            <div style="display:none">
                <xsl:attribute name="id">divTest_<xsl:value-of select="$fixture.name"/>
                </xsl:attribute>
    ...
    <xsl:template name="progressSegment">
        ...
        <xsl:if test="$value > 0">
            <div class="{$class}">
                <xsl:attribute name="style">width: <xsl:value-of select="floor($percent * 100) div 100"/>%
                </xsl:attribute>
                &#160;
            </div>
        </xsl:if>
