...
static partial class GamePad
{
 ...
 (capabilities.Flags & SharpDX.XInput.CapabilityFlags.FfbSupported) == SharpDX.XInput.CapabilityFlags.FfbSupported;
        ...
        GamePadDeadZone rightDeadZoneMode)
    {
        ...
        var thumbSticks = new GamePadThumbSticks(
            leftPosition: new Vector2(gamepad.LeftThumbX, gamepad.LeftThumbY) / short.MaxValue,
            rightPosition: new Vector2(gamepad.RightThumbX, gamepad.RightThumbY) / short.MaxValue,
    ...
    private static Buttons AddButtonIfPressed(
        ...
        Buttons xnaButton)
    {
        var buttonState = (buttonFlags & xInputButton) == xInputButton
            ? ButtonState.Pressed
    ...
    private static bool PlatformSetVibration(int index, float leftMotor, float rightMotor, float leftTrigger,
        float rightTrigger)
    {
        if (!_connected[index])
        {
