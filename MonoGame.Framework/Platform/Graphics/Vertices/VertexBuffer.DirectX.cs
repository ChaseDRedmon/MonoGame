...
public partial class VertexBuffer
{
    ...
    void CreatedCachedStagingBuffer()
    {
        ...
        stagingDesc.CpuAccessFlags =
            SharpDX.Direct3D11.CpuAccessFlags.Read | SharpDX.Direct3D11.CpuAccessFlags.Write;
        ...
        else
        {
            ...
            try
            {
                ...
                lock (GraphicsDevice._d3dContext)
                {
                    ...
                    {
                        SharpDX.Utilities.CopyMemory(dataPtr, box.DataPointer + offsetInBytes,
                            vertexStride * elementCount);
                    }
                    else
                    {
                        for (int i = 0; i < elementCount; i++)
                            SharpDX.Utilities.CopyMemory(dataPtr + i * TsizeInBytes,
                                box.DataPointer + i * vertexStride + offsetInBytes, TsizeInBytes);
                    }
    ...
    private void PlatformSetData<T>(int offsetInBytes, T[] data, int startIndex, int elementCount, int vertexStride,
        SetDataOptions options, int bufferSize, int elementSizeInBytes) where T : struct
    {
        GenerateIfRequired();
        ...
        if (_isDynamic)
        {
            ...
            lock (d3dContext)
            {
                ...
                {
                    for (int i = 0; i < elementCount; i++)
                        SharpDX.Utilities.Write(dataBox.DataPointer + offsetInBytes + i * vertexStride, data,
                            startIndex + i, 1);
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    lock (d3dContext)
                    {
                        ...
                        // Map the staging resource to a CPU accessible memory
                        var box = d3dContext.MapSubresource(_cachedStagingBuffer, 0,
