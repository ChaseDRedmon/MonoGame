...
class WinFormsGameWindow : GameWindow, IDisposable
{
    ...
    private static ReaderWriterLockSlim _allWindowsReaderWriterLockSlim = new(LockRecursionPolicy.NoRecursion);

    private static List<WinFormsGameWindow> _allWindows = new();
    ...
    internal WinFormsGameWindow(WinFormsGamePlatform platform)
    {
        ...
        ChangeClientSize(new Size(GraphicsDeviceManager.DefaultBackBufferWidth,
    ...
    [DllImport("user32.dll", ExactSpelling = true, CharSet = CharSet.Auto)]
    internal static extern int MapWindowPoints(HandleRef hWndFrom, HandleRef hWndTo, out POINTSTRUCT pt,
        int cPoints);
    ...
    private void UpdateMouseState()
    {
        ...
        MouseState.RightButton = (buttons & MouseButtons.Right) == MouseButtons.Right
            ? ButtonState.Pressed
            : ButtonState.Released;
        MouseState.XButton1 = (buttons & MouseButtons.XButton1) == MouseButtons.XButton1
            ? ButtonState.Pressed
            : ButtonState.Released;
        MouseState.XButton2 = (buttons & MouseButtons.XButton2) == MouseButtons.XButton2
            ? ButtonState.Pressed
            : ButtonState.Released;
