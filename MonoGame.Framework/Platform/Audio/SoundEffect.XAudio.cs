...
partial class SoundEffect
{
    ...
    private void PlatformInitializePcm(byte[] buffer, int offset, int count, int sampleBits, int sampleRate,
    ...
    private void PlatformInitializeFormat(byte[] header, byte[] buffer, int bufferSize, int loopStart,
        int loopLength)
    {
    ...
    private void PlatformInitializeXact(MiniFormatTag codec, byte[] buffer, int channels, int sampleRate,
        int blockAlignment, int loopStart, int loopLength, out TimeSpan duration)
    {
    ...
    private void PlatformLoadAudioStream(Stream stream, out TimeSpan duration)
    {
        ...
        catch (InvalidOperationException ex)
        {
            throw new ArgumentException(
                "Ensure that the specified stream contains valid PCM or IEEE Float wave data.", ex);
        }
        ...
        switch (soundStream.Format.Encoding)
        {
            ...
            case WaveFormatEncoding.IeeeFloat:
                sampleCount = (int)(dataStream.Length /
                                    (soundStream.Format.Channels * soundStream.Format.BitsPerSample / 8));
                break;
            default:
                throw new ArgumentException(
