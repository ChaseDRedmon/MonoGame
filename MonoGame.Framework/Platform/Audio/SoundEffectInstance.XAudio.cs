...
public partial class SoundEffectInstance : IDisposable
{
    ...
    private void PlatformApply3D(AudioListener listener, AudioEmitter emitter)
    {
        ...
        var dpsSettings = SoundEffect.Device3D.Calculate(l, e, CalculateFlags.Matrix | CalculateFlags.Doppler,
            srcChannelCount, dstChannelCount);

        // Apply Volume settings (from distance attenuation) ...
        _voice.SetOutputMatrix(SoundEffect.MasterVoice, srcChannelCount, dstChannelCount,
            dpsSettings.MatrixCoefficients, 0);
    ...
    internal void UpdateOutputMatrix()
    {
        ...
        if (!(_reverbMix > 0.0f))
            _voice.SetOutputMatrix(srcChannelCount, dstChannelCount,
                CalculateOutputMatrix(_pan, 1.0f, srcChannelCount));
        else
        {
            _voice.SetOutputMatrix(SoundEffect.ReverbVoice, srcChannelCount, dstChannelCount,
                CalculateOutputMatrix(_pan, _reverbMix, srcChannelCount));
            _voice.SetOutputMatrix(SoundEffect.MasterVoice, srcChannelCount, dstChannelCount,
                CalculateOutputMatrix(_pan, 1.0f - Math.Min(_reverbMix, 1.0f), srcChannelCount));
        }
    }
