...
public sealed partial class SoundEffect : IDisposable
{
    ...
    internal SoundEffect(byte[] header, byte[] buffer, int bufferSize, int durationMs, int loopStart,
        int loopLength)
    {
        Initialize();
        if (_systemState != SoundSystemState.Initialized)
        ...
        if (format == 1)
        {
            ...
            PlatformInitializePcm(buffer, 0, bufferSize, bitsPerSample, sampleRate, (AudioChannels)channels,
    ...
    internal SoundEffect(MiniFormatTag codec, byte[] buffer, int channels, int sampleRate, int blockAlignment,
        int loopStart, int loopLength)
    {
        ...
        if (codec == MiniFormatTag.Pcm)
        {
            ...
            PlatformInitializePcm(buffer, 0, buffer.Length, 16, sampleRate, (AudioChannels)channels, loopStart,
                loopLength);
            return;
        }

        PlatformInitializeXact(codec, buffer, channels, sampleRate, blockAlignment, loopStart, loopLength,
            out _duration);
    }
        ...
        int loopLength)
    {
        ...
        if ((count % blockAlign) != 0)
            throw new ArgumentException(
                "Ensure that the count meets the block alignment requirements for the number of channels.",
                "count");
