...
public partial class WaveBank : IDisposable
{
    ...
    private WaveBank(AudioEngine audioEngine, string waveBankFilename, bool streaming, int offset, int packetsize)
    {
        ...
        {
            wavebankdata.BankName =
                System.Text.Encoding.UTF8.GetString(reader.ReadBytes(16), 0, 16).Replace("\0", "");
        }
        else
        {
            wavebankdata.BankName =
                System.Text.Encoding.UTF8.GetString(reader.ReadBytes(64), 0, 64).Replace("\0", "");
        }
        ...
        if (!_streaming)
        {
            for (var i = 0; i < _streams.Length; i++)
            {
                ...
                _sounds[i] = new SoundEffect(codec, audiodata, channels, rate, alignment, info.LoopStart,
                    info.LoopLength);
            }
    ...
    private void DecodeFormat(int format, out MiniFormatTag codec, out int channels, out int rate,
        out int alignment)
    {
        if (_version == 1)
        {
