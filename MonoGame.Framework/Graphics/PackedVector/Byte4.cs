...
public struct Byte4 : IPackedVector<uint>, IEquatable<Byte4>, IPackedVector
{
    ...
    static uint Pack(ref Vector4 vector)
    {
        ...
        var byte4 = (uint)MathF.Round(Math.Clamp(vector.X, min, max)) & 0xFF;
        var byte3 = ((uint)MathF.Round(Math.Clamp(vector.Y, min, max)) & 0xFF) << 0x8;
        var byte2 = ((uint)MathF.Round(Math.Clamp(vector.Z, min, max)) & 0xFF) << 0x10;
        var byte1 = ((uint)MathF.Round(Math.Clamp(vector.W, min, max)) & 0xFF) << 0x18;
