...
public struct NormalizedShort4 : IPackedVector<ulong>, IEquatable<NormalizedShort4>
{
    ...
    private static ulong PackInFour(float vectorX, float vectorY, float vectorZ, float vectorW)
    {
        ...
        var word4 = (ulong)((int)MathF.Round(Math.Clamp(vectorX * maxPos, minNeg, maxPos)) & mask);
        var word3 = (ulong)((int)MathF.Round(Math.Clamp(vectorY * maxPos, minNeg, maxPos)) & mask) << 0x10;
        var word2 = (ulong)((int)MathF.Round(Math.Clamp(vectorZ * maxPos, minNeg, maxPos)) & mask) << 0x20;
        var word1 = (ulong)((int)MathF.Round(Math.Clamp(vectorW * maxPos, minNeg, maxPos)) & mask) << 0x30;
