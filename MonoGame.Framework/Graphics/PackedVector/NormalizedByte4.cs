...
public struct NormalizedByte4 : IPackedVector<uint>, IEquatable<NormalizedByte4>
{
    ...
    {
        var byte4 = ((uint)MathF.Round(Math.Clamp(x, -1.0f, 1.0f) * 127.0f) & 0xff) << 0;
        var byte3 = ((uint)MathF.Round(Math.Clamp(y, -1.0f, 1.0f) * 127.0f) & 0xff) << 8;
        var byte2 = ((uint)MathF.Round(Math.Clamp(z, -1.0f, 1.0f) * 127.0f) & 0xff) << 16;
        var byte1 = ((uint)MathF.Round(Math.Clamp(w, -1.0f, 1.0f) * 127.0f) & 0xff) << 24;
