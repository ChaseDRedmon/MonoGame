...
public struct Short4 : IPackedVector<ulong>, IEquatable<Short4>
{
    ...
    static ulong Pack(ref Vector4 vector)
    {
        ...
        var word4 = (ulong)(int)MathF.Round(Math.Clamp(vector.X, minNeg, maxPos)) & mask;
        var word3 = (ulong)((int)MathF.Round(Math.Clamp(vector.Y, minNeg, maxPos)) & mask) << 0x10;
        var word2 = (ulong)((int)MathF.Round(Math.Clamp(vector.Z, minNeg, maxPos)) & mask) << 0x20;
        var word1 = (ulong)((int)MathF.Round(Math.Clamp(vector.W, minNeg, maxPos)) & mask) << 0x30;
