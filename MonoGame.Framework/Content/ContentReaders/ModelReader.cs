    ...
    protected internal override Model Read(ContentReader reader, Model existingInstance)
    {
        ...
        for (int i = 0; i < meshCount; i++)
        {
            ...
            for (uint j = 0; j < partCount; j++)
            {
                ...
                int jj = (int)j;
                reader.ReadSharedResource<VertexBuffer>(delegate(VertexBuffer v) { parts[jj].VertexBuffer = v; });
                reader.ReadSharedResource<IndexBuffer>(delegate(IndexBuffer v) { parts[jj].IndexBuffer = v; });
                reader.ReadSharedResource<Effect>(delegate(Effect v) { parts[jj].Effect = v; });
            }
