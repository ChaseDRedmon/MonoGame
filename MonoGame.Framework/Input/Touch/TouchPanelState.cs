...
public class TouchPanelState
{
        ...
        for (var i = 0; i < state.Count; i++)
        {
            ...
            if (existingTouch.Id == touch.Id)
            {
                ...
                if (existingTouch.State == TouchLocationState.Pressed &&
                    touch.State == TouchLocationState.Released && existingTouch.PressTimestamp != touch.Timestamp)
                {
                    state.RemoveAt(i);
                }
                else
    ...
    public TouchCollection GetState()
    {
        ...
        for (var i = _touchState.Count - 1; i >= 0; i--)
        {
            ...
            if (touch.SameFrameReleased && touch.Timestamp < CurrentTimestamp &&
            ...
            foreach (var touch in temp)
            {
                if (touch.State != TouchLocationState.Released)
                    ApplyTouch(_touchState,
                        new TouchLocation(touch.Id, TouchLocationState.Released, touch.Position, CurrentTimestamp));
            }
